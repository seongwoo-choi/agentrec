...
記録する。正規化されたアクションのタイムライン、監督下にあったプロセスの結果、
実行区間の前後でのリポジトリの差分、そしてリポジトリ自身が固定したチェックの
結果である。それぞれは異なる観測者に由来し、バンドルはそれらを分けて保持する。
...
- **スコアも、勝者も、推奨もない。** 比較は記録されたフィールドを示すだけで、そこ
  ...
  ていないため、比較はそれらを表示しない。
- **コミットされたバイトのみ。** リンクされたワークツリーが受け取るのは `HEAD` が
  保持しているものである。追跡されていない `.env` ファイルやローカルの資格情報は、
  その中にコピーされない。プロバイダー CLI は自身の既存の認証とキャッシュを使う。
  agentrec は資格情報の輸送を一切追加せず、ワークスペース準備のステップも実行しない
  ので、セットアップがコミットされていないプロジェクトは、各レグがそれなしで始める
  プロジェクトである。
...
- **タスクは 1 つのコマンドライン引数である。** タスクファイルは一度だけ読まれる —
  ...
  エージェントに `claude -p <task>` と `codex exec <task>` として渡される。標準入力
  で与えるプロンプトや、複数の引数に分けたプロンプトは、ここではサポートされない。
- **検証は必須であり、レグは直列化される。** 両方の実行はコミットされた
  `.agentrec.yaml` に対して検証され、片方ずつ順に実行されるので、チェックとそれが
  触れる外部状態が重なることは決してない。
- **ソースのチェックアウトは見つけたときのまま残される。** それはクリーンでなければ
  ならず、コマンドの実行中ずっとロックされ、その `HEAD`、ステータス、参照、ワーク
  ツリー一覧、追跡されているバイトは実行後も変わらない — 成功したときも、失敗した
  ときも、中断されたときも。
...
両方の検証が通れば `0`、レグが失敗した・未完了で終わった・チェックアウトを削除でき
なかった場合は `1`、実行が中断された場合は `130` である。**プロバイダー自身の終了
コードはそのバンドル内の証拠であり、集約コマンドがそれをそのまま通すことは決して
ない。**

中断が発生すると、現在のレグのプロセスグループを停止し、そのレグの証拠を確定し、
チェックアウトを削除し、まだ開始していなかったレグは決して起動しない。そのとき比較
は、実行されなかったランナーを `(not run)` として示す。agentrec が即座に殺された
...
`agentrec shadow run` はその証拠の対象では**ない**。上に述べたその挙動 — 隔離、
後片付け、シグナル処理、終了コード、比較出力 — は、代役のプロバイダーを使うリポジトリ
のテストが裏づけている。実物の Claude Code と Codex の CLI に対しては、まだ記録され
ていない。
