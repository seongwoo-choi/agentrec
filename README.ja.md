...
| 📋 `agentrec list [--cwd <path>] [--exit-reason <reason>] [--verification-status <status>] [--failures-only]` | 実行を新しい順に一覧します。`--failures-only` は明示的なプロセス失敗と、一貫して成功していない終了済みの検証証拠だけを残し、保留中および中立の結果は除外します。`--cwd` とは併用できますが、厳密ステータスフィルターとは併用できません。 |
