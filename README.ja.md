...
> **ステータス:** 最新リリースは v0.15.0 です。ビューアーが「何が起きたか」の記録として
> 読めるようになりました: 読み取り優先のタイムライン、折りたたまれたフックとツールのノイズ、
> 安全なタイトル、所要時間、ターンの序数、エージェントの最後のメッセージ — いずれも変わらない
> 証拠の上の折りたたみ・ラベル・位置にすぎません。
>
...
**v0.15.0 — タイトル優先の run 一覧:** 読み込み済みのサイドバー行は、64 KiB
...
**v0.15.0 — 証拠を読みやすい画面に:** 詳細画面の見出しには、読み込み済みの要約にある
...
**v0.15.0 — 読み取り優先のアクションタイムライン:** アクションは既定で
...
**v0.15.0 — 変更とプロバイダーイベントを読みやすく:** 変更タブの既定の
...
**v0.15.0 — プロンプト行に何ターン目かを表示:** ローカルストアのマルチターン
セッションでは 108 件のアクションの間に 12 件のプロンプト行があり、スクロールしながら
数えなければ何番目のリクエストか、あと何件続くかが分かりませんでした。実行詳細は今後
`promptCount`(記録中の `user.prompt` アクション数。アクションを数える既存のパスで一緒に
集計)を持ち、各プロンプト行の話者行は `自分 · 3 / 12` のように、これまでに読み込んだ
プロンプト中の順位を示します。ページは順に届くのでこの順位は正確です。タスク通知も
プロバイダーがプロンプトとして記録したためターンとして数えます。プロンプトが 1 件だけの
実行では序数を表示しません。どの返答がどのリクエストへの答えかは引き続き推測しません。

**v0.15.0 — タスク通知はオペレーターの発話ではない:** Claude Code は完了した
バックグラウンドタスクを、オペレーター自身の言葉と同じプロンプトフック経由で
`<task-notification>` で始まるプロンプトとして会話に戻します。ローカルストア唯一の
マルチターンセッションでは、プロンプト行 12 件のうち 4 件がこの通知で、すべて **自分**
と表示されていました。`<task-notification>` で始まるプロンプトは今後、中立的なトーンの
**タスク通知** と表示されます。本文はそのままで、`user.prompt` 記録・id・状態・検索・
インスペクターは変わりません。それ以外のプロンプトは引き続き **自分** です。EN/KO/JA/ZH
対応。

**v0.15.0 — 証拠リンクの説明文はボタンの下に:** アクションを選択するたびに、コピー
リンクがローカル用であることを説明する 2 行の文がボタンとペイロードの間にありました。
今後は同じ文言・同じ 4 言語でボタンのツールチップとアクセシブルな説明になり、
`コピーしました`/クリップボード拒否の状態表示と代替 URL 欄はそのまま残ります。計測した
実行では最初のペイロードが 43 px 上がります。

**v0.15.0 — 実行見出しは最初の文で止まる:** ローカルストアではタイトル 35 件のうち
22 件が 120 文字の上限で語句の途中で切れ、27 件の見出しが 2 行に折り返していましたが、
その多くは最初の文が 80 文字以内で終わり、残りはリクエストの 2 文目・3 文目で、
リクエストカードに全文があります。見出しは今後、読み込んだタイトルの最初の文の終わり
(12 文字目以降の `. `、`? `、`! `)までを表示します。コロンはリクエストの本文を導入するため
境界にしません。完全なタイトルは見出しのツールチップ、サイドバーの行、リクエストカードに
残ります。後続の文は省かれますが、単語が切られることはありません。このストアでは 2 行の
見出しが 27 件から 5 件に減りました。

**v0.15.0 — タイムラインパネルが画面に収まる:** タイムラインとインスペクターの
パネルは固定の推定値(`100vh − 340px`)で高さが決まっていましたが、上のコンテキストは
実測 565–583 px あり、どのデスクトップサイズでもパネルの下端がビューポートより 225–383 px
下にあって、ひとつのリストにスクロールバーが二つ必要でした。デスクトップレイアウトでは
パネルが実行コンテキストの下に残る高さに合わせて伸縮し、最小 320 px を保ちます。
1920×1080 では通常の実行でページスクロールがなくなり、高さ 900 px のウィンドウでは
パネルは最小高さに留まります — コンテキストが残す空間の正直な限界です。狭いレイアウトは
変わりません。

**v0.15.0 — コンパクトな要約ストリップ:** タイムライン上の要約グリッドは 9 列のうち
3 列が常に空で、6 枚のカードのうち 2 枚はすぐ下のタブラベルに表示されるアクション数・
イベント数を繰り返していました。今後はプロセス結果・検証判定・リポジトリ証拠・警告の
4 枚を 4 列いっぱいに表示し、件数はタブラベルに残ります。375 px ではストリップが 60 px
短くなります。折りたたんだり隠したりしたものはありません。

**v0.15.0 — Codex パッチ行にファイル名を表示:** Codex の編集はすべて
...
**v0.15.0 — イベント要約でフックのライフサイクル記録を折りたたみ:** ローカル
...
**v0.15.0 — リクエストの隣にエージェントの最後のメッセージ:** リクエストカードは
...
**v0.15.0 — スキップリンク:** 開いた直後は最初の run 行まで Tab を 9 回押す必要が
...
**v0.15.0 — run 一覧の所要時間:** 読み込んだ各 run 行に、開始時刻の横で記録された
...
**v0.15.0 — 後からの検証を見つけやすく:** `--allow-run` なしで起動したビューアは
...
**v0.15.0 — 読み込んだ実行の概要:** run 一覧に、折りたたまれた**読み込んだ実行の
