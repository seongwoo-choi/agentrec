...
[リリースノート](docs/releases/v0.9.0.md) ·
...
> **ステータス:** v0.9.0 が最新リリースです。ローカル自動化で機械可読な失敗調査を
> 最後まで実行できるようになりました。`agentrec list --json` で run を見つけ、
> `agentrec show <run-id> --json` または `--failures-only` との組み合わせで、サイズを
> 制限して無害化した証拠を取得できます。プロバイダーの未加工ペイロードは出力しません。
>
...
```sh
archive=agentrec_0.9.0_darwin_arm64.tar.gz
awk -v file="$archive" '$2 == file { print }' SHA256SUMS | shasum -a 256 -c -
tar -xzf "$archive"
./agentrec_0.9.0_darwin_arm64/agentrec version
```

```sh
go install github.com/seongwoo-choi/agentrec/cmd/agentrec@v0.9.0
```
...
- [v0.9.0 のリリースノート](docs/releases/v0.9.0.md) · [v0.8.0](docs/releases/v0.8.0.md) · [v0.7.1](docs/releases/v0.7.1.md) · [v0.7.0](docs/releases/v0.7.0.md) · [v0.6.0](docs/releases/v0.6.0.md) · [v0.5.0](docs/releases/v0.5.0.md) · [v0.4.0](docs/releases/v0.4.0.md) · [v0.3.0](docs/releases/v0.3.0.md) · [v0.2.0](docs/releases/v0.2.0.md) · [v0.1.0](docs/releases/v0.1.0.md)
...
gofmt -l .
go build ./...
scripts/build-release.sh v0.9.0 "$(git rev-parse HEAD)" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" dist
```
