...
使用量・検証の証拠を 1 画面で確認できます。親操作 ID（`parentId`）を持つ子エージェントの
作業はインデントして表示しますが、プロバイダーが報告した関係を OS が観測した因果関係とは
主張しません。操作またはイベントを選ぶと、サニタイズ済みの `input`/`result` を実行可能な
HTML ではなくテキストとして確認できます。

各ビューアースナップショットは、1 つの実行ディレクトリを固定し、操作・イベントストリームの
変更されないバイトコピーを作成します。作成中に表示対象の artifact が変わった場合はスナップショットを拒否します。API は両
ストリームを最大 250 件、目標 1 MiB ごとに返します。正常な 1 レコードが目標サイズを超える
場合はありますが、上限サイズのバンドル全体を一度に転送・描画することはありません。
