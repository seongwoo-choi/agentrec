...
`agentrec view [<run-id>|latest]` は、同じサイズ制限付きのバンドル読み取り経路を使って、
ローカルの読み取り専用 Web UI を開きます。実行一覧、記録された依頼、正規化済みの操作
タイムライン、サニタイズ済みのプロバイダーイベント、帰属を分離したプロセス・リポジトリ・
使用量・検証の証拠を 1 画面で確認できます。
概要には、プロセスの結果と所要時間、検証判定、検証済みのリポジトリ状態と変更集計、
操作・イベント・警告の件数をまとめて表示します。`Changes` タブでは追跡対象・未追跡のパスを
一覧し、追跡対象のパスに記録されたサイズ制限・サニタイズ済みパッチを開けます。リポジトリの
証拠は実行中の観測であり、因果関係の証明ではないことを明記します。
ファイル操作の明示的に正規化された入力パスが表示中の変更パスと完全に一致する場合、
`same path observed — not causal proof` と表示します。コマンドや結果のテキストからパスを推測しません。
recorder は実行時のファイルシステム namespace が存在する間に明示的なパスをリポジトリ相対の
`repositoryPaths` に変換します。そのため viewer は live リポジトリを開き直さずに symlink alias を扱えます。
親操作 ID（`parentId`）を持つ子エージェントの
作業はインデントして表示しますが、プロバイダーが報告した関係を OS が観測した因果関係とは
主張しません。操作またはイベントを選ぶとサニタイズ済みの `input`/`result` を、変更ファイルを
選ぶとリポジトリのメタデータとサイズ制限付きパッチを確認できます。すべての payload は実行可能な
HTML ではなくテキストとして表示されます。

各ビューアースナップショットは、1 つの実行ディレクトリを固定し、操作・イベントストリームと
tracked patch の変更されないバイトコピーを作成して、索引化した Changes 文書を検証します。
作成中に表示対象の artifact が変わった場合はスナップショットを拒否します。API は操作・イベント・
変更一覧を 1 ページ最大 250 件、目標 1 MiB で返し、patch ページは 1 MiB に制限します。正常な
1 レコードがストリームの目標サイズを超える場合はありますが、上限サイズのバンドル全体を一度に
転送・描画することはありません。

既定では `127.0.0.1` のランダムなポートに接続し、既定のブラウザーを開きます。ブラウザーを
開かず URL だけを表示するには `--no-open`、固定のループバックポートを使うには `--listen
127.0.0.1:<port>` を指定します。ループバック以外のアドレスは拒否され、ビューアーにはデータを
変更するエンドポイントも外部リソースもありません。
...
- **スコア、勝者、推奨はない。** 比較は記録済みフィールドを示すだけで、そこから導出した判断は示しません。どちらの実行を選ぶかは読み手が判断します。プロバイダー報告の使用量は各レグの provider と `run` または `session` の scope を明示して個別に表示し、プロバイダー間で合算したり同等の値として扱ったりしません。
