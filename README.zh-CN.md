...
| 📋 `agentrec list [--cwd <path>] [--exit-reason <reason>] [--verification-status <status>] [--failures-only]` | 按时间倒序列出运行记录。`--failures-only` 仅保留明确的进程失败与未能一致通过的终态验证证据，并省略待定和中性结果。它可与 `--cwd` 组合，但不能与精确状态筛选器同时使用。 |
