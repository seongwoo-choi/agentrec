...
agentrec 不是交互式记录界面，不是云端遥测服务，也不能证明智能体导致了所有观察到的文件变更。它是围绕一次非交互式运行建立的本地证据边界；其价值恰恰在于明确说明观察到了什么、由谁观察到，以及无法证明什么。
...
- **没有评分、胜者或推荐。** 比较只展示记录到的字段，不会从中推导其他结论。应偏好哪次运行由读者自行判断。目前提供方报告的成本和 token 字段不在记录证据中，因此比较不会显示它们。
