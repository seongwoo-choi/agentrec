...
> **状态：** v0.14.0 是最新发布版本。查看器的全局运行搜索现在可以找到已保存的
...
**未发布——标题优先的运行列表：** 已加载的侧栏条目现在会优先显示标题。标题取自完整、
...
**未发布——更易阅读的证据工作区：** 详情页优先使用已加载摘要中的安全标题，并在下方保留
...
**未发布——阅读优先的操作时间线：** 操作现在默认以**阅读视图**打开。只有同一父级下、
...
**未发布——更易阅读的变更与提供方事件：** 变更标签页默认采用**文件夹视图**，按实际路径的
...
**未发布——提示行标明是第几轮：** 在本地存储的一个多轮会话中，108 个操作之间散布着
...
**未发布——任务通知并非操作者所说：** Claude Code 会把已完成的后台任务通过与操作者
...
**未发布——时间线面板适配所在屏幕：** 时间线和检查器面板此前按固定估值（`100vh − 340px`）
...
**未发布——紧凑的摘要条：** 时间线上方的摘要网格有 9 列，其中 3 列始终为空，6 张卡片中
有 2 张重复了正下方标签页标签上的操作数和事件数。现在保留进程结果、验证结论、仓库证据、
警告 4 张卡片并占满 4 列；计数保留在标签页标签中。在 375px 下摘要条缩短 60px。没有任何
内容被折叠或隐藏。

**未发布——Codex 补丁行显示文件名：** 每次 Codex 编辑都以 `apply_patch` 文档形式记录在
...
**未发布——事件摘要折叠钩子生命周期记录：** 在本地存储的 17 个 Claude 运行中，
...
**未发布——请求旁边显示代理的最后一条消息：** 请求卡片展示了提出的要求，但代理最后说了
...
**未发布——跳转链接：** 刚打开时需要按九次 Tab 才能到达第一条运行。现在**跳到运行列表**和
**跳到运行证据**两个链接排在最前，仅在获得焦点时显示。激活链接会把焦点移到目标处，下一次
Tab 从那里继续。其余的 Tab 顺序、折叠区域和焦点恢复均不变。

**未发布——运行行显示耗时：** 每条已加载的运行行会在开始时间旁边显示所记录进程运行了
...
**未发布——更易发现的事后验证：** 未带 `--allow-run` 启动的查看器此前会默默省略**立即验证**
...
**未发布——已加载运行一览：** 运行列表新增一个折叠的**已加载运行一览**面板，按提供方、
