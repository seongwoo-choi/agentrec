...
- **점수도, 승자도, 추천도 없다.** 비교는 기록된 필드를 보여줄 뿐, 거기서 파생된
  ...
  비교는 그것들을 보여주지 않는다.
- **커밋된 바이트만.** 연결된 워크트리는 `HEAD`가 담고 있는 것을 받는다. 추적되지
  않는 `.env` 파일과 로컬 자격 증명은 그 안으로 복사되지 않는다. 제공자 CLI는
  자신의 기존 인증과 캐시를 사용한다. agentrec은 어떤 자격 증명 전달도 추가하지
  않고 워크스페이스 준비 단계도 실행하지 않으므로, 설정이 커밋되어 있지 않은
  프로젝트는 각 레그가 그것 없이 시작하는 프로젝트다.
...
- **작업은 하나의 명령줄 인자다.** 작업 파일은 한 번만 읽힌다 — 최대 64 KiB의,
  ...
  `claude -p <task>`와 `codex exec <task>`로 전달된다. stdin으로 주는 프롬프트나
  여러 인자에 나누어 주는 프롬프트는 여기서 지원되지 않는다.
- **검증은 필수이며, 레그는 직렬화된다.** 두 실행 모두 커밋된 `.agentrec.yaml`에
  대해 검증되고, 하나가 끝난 뒤 다른 하나가 실행되므로 검사와 그것이 건드리는
  외부 상태는 결코 겹치지 않는다.
- **소스 체크아웃은 발견된 그대로 남는다.** 그것은 깨끗해야 하고, 명령이 실행되는
  내내 잠기며, 그 `HEAD`, 상태, 레퍼런스, 워크트리 목록, 추적되는 바이트는 이후에도
  변하지 않는다 — 성공했을 때도, 실패했을 때도, 중단되었을 때도.
...
통과하면 `0`, 레그가 실패했거나 불완전하게 끝났거나 체크아웃을 제거할 수 없었으면
`1`, 실행이 중단되었으면 `130`이다. **제공자 자신의 종료 코드는 그 번들 안의
증거이며, 집계 명령이 그것을 그대로 내보내는 일은 결코 없다.**

중단이 발생하면 현재 레그의 프로세스 그룹을 멈추고, 그 레그의 증거를 마감하고,
체크아웃을 제거하며, 아직 시작하지 않은 레그는 절대 실행하지 않는다. 그러면
