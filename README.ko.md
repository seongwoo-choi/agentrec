...
[릴리스 노트](docs/releases/v0.14.0.md) ·
...
> **상태:** v0.14.0이 최신 릴리스입니다. 뷰어에서 모든 실행을 검색하면 저장된
...
**미출시 — 제목 우선 실행 목록:** 이제 로드된 사이드바 행은 64 KiB 이하의 완전하고
...
**미출시 — 증거를 읽기 편한 화면:** 상세 화면은 로드된 요약의 안전한 제목을 먼저
...
**미출시 — 읽기 우선 액션 타임라인:** 액션은 이제 기본적으로 **읽기 보기**로 열립니다.
...
**미출시 — 읽기 편한 변경·프로바이더 이벤트:** 변경 탭은 기본 **폴더 보기**에서
...
**미출시 — 프롬프트 행에 몇 번째 턴인지 표시:** 로컬 저장소의 다중 턴 세션에서 액션
...
**미출시 — 작업 알림은 사용자의 발화가 아닙니다:** Claude Code는 끝난 백그라운드
...
**미출시 — 증거 링크 설명이 버튼 아래로 들어갑니다:** 액션을 선택할 때마다 복사
...
**미출시 — 실행 제목은 첫 문장에서 멈춥니다:** 로컬 저장소에서 제목 35개 중
...
**미출시 — 타임라인 패널이 화면에 맞춰집니다:** 타임라인·인스펙터 패널은 고정
...
**미출시 — 요약 띠 압축:** 타임라인 위 요약 격자는 9열 중 3열이 항상 비어 있었고,
...
**미출시 — Codex 패치 행에 파일 이름 표시:** Codex 편집은 모두 `input.command`에
...
**미출시 — 이벤트 요약에서 훅 수명주기 기록 접기:** 로컬 저장소의 Claude 실행
...
**미출시 — 요청 옆의 에이전트 마지막 메시지:** 요청 카드는 무엇을 물었는지 보여줬지만,
...
**미출시 — 건너뛰기 링크:** 처음 열면 첫 실행 행까지 Tab을 아홉 번 눌러야 했습니다.
...
**미출시 — 실행 목록의 소요 시간:** 불러온 각 실행 행에 시작 시각 옆으로 기록된
...
**미출시 — 나중 검증 안내:** `--allow-run` 없이 시작한 뷰어는 **지금 검증** 버튼을
...
**미출시 — 불러온 실행 한눈에 보기:** 실행 목록에 접힌 **불러온 실행 한눈에 보기**
