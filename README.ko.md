...
`<run>/report.md`에 쓴다. 단 한 번뿐이며 다시 쓰지 않는다. 그 이름의 리포트가 이미
존재하면 덮어쓰지 않고 거부한다.
...
- **연결된 워크트리는 보안 경계가 아니다.** 공통 Git 디렉터리와 레퍼런스를 공유하며,
  ...
  관찰된 drift가 있으면 다음 레그를 시작하지 않고 `1`로 끝낸다. 단, 실행도 중단된
  경우에는 `130`이 우선한다. agentrec은 drift를 보고할 뿐 파괴적으로 복구하지 않는다.
...
체크아웃을 제거할 수 없었으면 `1`, drift도 함께 관찰됐더라도 실행이 중단되었으면
`130`이다. **제공자 자신의 종료 코드는 그 번들 안의
