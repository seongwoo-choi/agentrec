...
최신 태그 릴리스는 `v0.1.0`입니다. 이 버전에는 `shadow run`, `events`, `view`가
들어 있지 않습니다. 아래에서 설명하는 세 명령은 아직 릴리스되지 않았으며 `main`에만
있습니다. 이 README의 모든 명령을 사용하려면 기존 `main` 체크아웃에서 현재 소스를
설치하세요.
