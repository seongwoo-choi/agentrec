...
- Selecting a group applies the matching existing run-list filter rather than navigating or creating a new view, so the overview and the list can never disagree. Selecting the active group clears that filter, matching its toggle semantics. The active project and verification groups expose their pressed state visually and to assistive technology; the filter, its URL parameter, the advanced-filter applied count and the loaded-scope note keep their current meaning.
