...
- Each `user.prompt` row's speaker line gains an ordinal — `You · 3 of 12` — where the ordinal is the row's rank among all `user.prompt` actions in the record and the total is `promptCount`. The snapshot scan projects that rank onto each prompt, so a page opened directly from a nonzero byte cursor retains its record-wide position. Task notifications (section 22) count as turns too, since the provider recorded them as prompts; their label reads `Task notification · 7 of 12`. A run with a single prompt shows no ordinal: `1 of 1` says nothing.
